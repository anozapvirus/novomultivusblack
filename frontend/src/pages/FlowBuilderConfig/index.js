...
import FlowBuilderIntegrationModal from "../../components/FlowBuilderIntegrationModal";
...
export const FlowBuilderConfig = () => {
  ...
  const [modalAddIntegration, setModalAddIntegration] = useState(null);
  ...
  const integrationAdd = data => {
    console.log("Adicionando novo nó de integração com dados:", data);
    addNode("integration", data);
  };

  const handleIntegrationSave = (newData) => {
      console.log("handleIntegrationSave chamado com dados:", newData);
      console.log("Estado atual de modalAddIntegration:", modalAddIntegration);
      console.log("Nó atual sendo editado (dataNode):", dataNode);
      
      if (modalAddIntegration === 'edit' && dataNode) {
          ...
          console.log("Nó atualizado pronto para updateNode:", nodeToUpdate);
          updateNode(nodeToUpdate);
          setModalAddIntegration(null);
          setDataNode(null);
      } else {
          console.log("Modo criação detectado.");
          integrationAdd(newData);
          setModalAddIntegration(null);
      }
  };
  ...
  const doubleClick = (event, node) => {
    ...
    if (node.type === "integration") {
      console.log("Abrindo modal de integração em modo edição");
      setModalAddIntegration("edit");
    }
  };
  ...
  const clickActions = type => {
    switch (type) {
      ...
      case "ticket":
        ...
        break;
      case "integration":
        console.log("Abrindo modal de integração em modo criação");
        setDataNode(null);
        setModalAddIntegration("create");
        break;
      default:
    }
  };
  ...
  return (
    <Stack sx={{ height: "100vh" }}>
      ...
      />
      <FlowBuilderIntegrationModal
        open={modalAddIntegration === 'create' || modalAddIntegration === 'edit'}
        onClose={() => { 
            setModalAddIntegration(null); 
            setDataNode(null);
        }}
