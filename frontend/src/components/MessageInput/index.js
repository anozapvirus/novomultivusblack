...
// Debounce para evitar muitas verificações ortográficas
let spellingTimeout = null;
...
const MessageInput = ({ ticketId, ticketStatus, droppedFiles, contactId, ticketChannel }) => {
  ...
  const handleChangeInput = async (e) => {
    ...
    // Debounce de 500ms para evitar muitas verificações
    spellingTimeout = setTimeout(async () => {
      if (value.length > 2) {
        try {
  ...
  const handleSuggestionClick = async (wrongWord, suggestion) => {
    try {
      ...
      const newMessage = inputMessage.replace(regex, suggestion);
      setInputMessage(newMessage);
      
      if (newMessage.length > 2) {
        const misspelledWords = await getMisspelledWords(newMessage);
        setMisspelled(misspelledWords);
      } else {
        setMisspelled([]);
      }
    } catch (error) {
      console.error('Erro ao corrigir palavra:', error);
      setMisspelled([]);
    }
  };
                            ...
                            {m.suggestions.length > 0 && (
                              <>
                                ...
                                {m.suggestions.slice(0, 3).map((s, i) => (
                                  <button
                                    key={i}
                                    style={{ marginLeft: 4, cursor: 'pointer', color: 'blue', border: 'none', background: 'none', textDecoration: 'underline' }}
                                    onClick={() => handleSuggestionClick(m.word, s)}
                                  >
                                    {s}
                                  </button>
                                ))}
