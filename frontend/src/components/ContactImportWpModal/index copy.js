...
const ContactImportWpModal = ({ isOpen, handleClose, selectedTags, hideNum, userProfile }) => {
  ...
  const initialContact = { name: "", remoteJid: "", error: "" }
        ...
        setTimeout(async () => {
          try {
            ...
            await api.post(`/contactsImport`, {
              name: item.name,
              number: item.remoteJid.toString(),
              email: item.email,
            });

            setCurrentContact({ name: item.name, remoteJid: item.remoteJid, error: "success" })
          } catch (err) {
            setCurrentContact({ name: item.name, remoteJid: item.remoteJid, error: err })
          }
        }, 330 * index);
      });
    }
  ...
  const handleOnExportContacts = async (model = false) => {
    ...
    const exportData = allDatas.map((e) => {
      return { name: e.name, remoteJid: (hideNum && userProfile === "user" ? e.isGroup ? e.remoteJid : e.remoteJid.slice(0, -6) + "**-**" + e.remoteJid.slice(-2) : e.remoteJid), email: e.email, tags: e.tags };
    });
                  ...
                  <div className={classes.label}>
                    ...
                    {currentContact?.name ?
                      <Button
                        ...
                        variant="text"
                      >
                        {`${currentContact?.name} => ${currentContact?.remoteJid} `}
                      </Button>
                      : <></>}
                  </div>
                </> :
