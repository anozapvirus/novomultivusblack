...
import notifyPendingTicketsJob from './jobs/notifyPendingTickets';
...
// Agendar job de notificação de tickets pendentes a cada 10 minutos
cron.schedule("*/10 * * * *", async () => {
  try {
    await notifyPendingTicketsJob();
  } catch (error) {
    logger.error('[notifyPendingTicketsJob]', error);
  }
});
