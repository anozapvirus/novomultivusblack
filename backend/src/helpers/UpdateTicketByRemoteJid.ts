...
export const updateTicketByRemoteJid = async (remoteJid: string, queue: number, user: number, statusText: string, unread: number): Promise<Response> => {
  // Validar e garantir que unread seja um número válido
  const validUnread = typeof unread === 'number' && !isNaN(unread) ? unread : 0;
  ...
  messages.forEach(async (message) => {
    ...
    await ticket.update({ status: statusText, queueId: queue, userId: user, unreadMessages: validUnread });
