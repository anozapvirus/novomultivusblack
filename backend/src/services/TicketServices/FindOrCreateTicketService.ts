...
const FindOrCreateTicketService = async (
  ...
  // Validar e garantir que unreadMessages seja um número válido
  const validUnreadMessages = typeof unreadMessages === 'number' && !isNaN(unreadMessages) ? unreadMessages : 0;
  // try {
  // let isCreated = false;
  ...
  if (ticket) {
    if (isCampaign) {
      ...
      await ticket.update({ unreadMessages: validUnreadMessages, isBot: false });
    }
  ...
  if (!ticket && timeCreateNewTicket !== 0) {
    ...
    if (ticket && ticket.status !== "nps") {
      await ticket.update({
        status: "pending",
        unreadMessages: validUnreadMessages,
        companyId,
