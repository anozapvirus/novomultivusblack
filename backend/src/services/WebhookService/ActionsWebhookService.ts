...
export const ActionsWebhookService = async (
  ...
  try {
    ...
    for (var i = 0; i < lengthLoop; i++) {
      ...
      if (nodeSelected.type === "integration") {
        console.log("Processando nó de integração:", nodeSelected.data);
        
        if (!nodeSelected.data.integrationId) {
          console.error("Nó de integração sem ID da integração");
          continue;
        }
        
        try {
          const integration = await QueueIntegrations.findByPk(nodeSelected.data.integrationId);
          
          if (!integration) {
            console.error(`Integração não encontrada: ${nodeSelected.data.integrationId}`);
            continue;
          }
          
          console.log(`Integração encontrada: ${integration.name}, tipo: ${integration.type}`);
          
          let ticketObj = null;
          if (idTicket) {
            ticketObj = await Ticket.findByPk(idTicket);
          }
          
          if (!ticketObj) {
            console.error("Ticket não encontrado para integração");
            continue;
          }
          
          const messageForIntegration = pressKey || ticketObj.lastMessage || "";
          console.log(`Mensagem a ser usada para a integração: "${messageForIntegration}"`);

          const contactData = await Contact.findOne({ where: { id: ticketObj.contactId } });
          
          if (!contactData) {
            console.error("Contato não encontrado para integração");
            continue;
          }
          ...
          const fakeMsg = {
             key: { 
               remoteJid: contactData.remoteJid || `${contactData.number}@s.whatsapp.net`, 
               id: randomString(16),
               fromMe: false 
             },
             messageTimestamp: Math.floor(Date.now() / 1000),
             message: { conversation: messageForIntegration }
          } as unknown as proto.IWebMessageInfo;

          console.log(`Chamando handleMessageIntegration para tipo: ${integration.type}`);
          ...
          await ticketObj.update({
            lastMessage: `Integração ${integration.type} processada (${messageForIntegration})`,
            useIntegration: true
          });
          
          if (integration.type === "gemini") {
            console.log("Integração Gemini processada. Interrompendo fluxo do FlowBuilder.");
            await ticketObj.update({
              lastFlowId: null,
              flowWebhook: false,
              dataWebhook: { status: "stopped" },
              isBot: true
            });
            break;
          }
          
          const nextConnections = connects.filter(conn => conn.source === nodeSelected.id);
          if (nextConnections.length > 0) {
            next = nextConnections[0].target;
          } else {
            console.log("Fim do fluxo após nó de integração (não-Gemini) sem conexão de saída");
             await ticketObj.update({
               lastFlowId: null, 
               flowWebhook: false, 
               dataWebhook: { status: "stopped" } 
          ...
          console.error("Erro detalhado ao processar nó de integração:", error);
          logger.error({ message: "Erro ao processar nó de integração", error });
        }
        continue;
      }
      ...
      if (nodeSelected.type === "singleBlock") {
        for (var iLoc = 0; iLoc < nodeSelected.data.seq.length; iLoc++) {
          ...
          if (elementNowSelected.includes("interval")) {
            console.log(`Processando nó de intervalo: ${nodeSelected.data.elements.filter(
              item => item.number === elementNowSelected
            )[0].value} segundos`);
            await intervalWhats(nodeSelected.data.elements.filter(
              item => item.number === elementNowSelected
            )[0].value);
            
            // Determinar o próximo nó após o intervalo
            const nextConnections = connects.filter(conn => conn.source === nodeSelected.id);
            if (nextConnections.length > 0) {
              next = nextConnections[0].target;
              console.log(`Próximo nó após intervalo: ${next}`);
            } else {
              console.log("Fim do fluxo após nó de intervalo sem conexão de saída");
              // Parar o fluxo se não houver conexão de saída
               if (ticket) {
                 await ticket.update({
                   lastFlowId: null, 
                   flowWebhook: false, 
            ...
            // Essencial para garantir que o loop avance para a PRÓXIMA iteração
            // antes de processar o nó seguinte (que foi definido na variável 'next')
            continue; 
          }
